...
@testset "IntervalSpecialFunctions tests" begin
    ...
    @testset "erfinv" begin
        ...
        @test erfinv(0..0.99) == Interval(-2.3390206794146806e-29, 1.8213863677184525)
        @test erfinv(-1/2..0).lo == -erfinv(0..1/2).hi
        @test erfinv(@interval(0.3)) == Interval(0.27246271472675426, 0.2724627147267545)
    ...
    @testset "erfcinv" begin
        ...
        @test erfcinv(1..3/2) == Interval(-0.4769362762044698, -2.5678235495712626e-21)
    ...
    @testset "gamma" begin
        @test gamma(1..3/2) == Interval(0.39359696954488754, 1.7760711647570133)
        @test gamma(@interval(1.3)) == Interval(0.8974706963062764, 0.8974706963062781)
    end
end
